...
- **Secondary**: If labels are required (e.g., data dashboards), treat them as supporting content (smaller, lighter weight, uppercase with letter-spacing) and emphasize the _data_.
...
- **Destructive**: If a "Delete" button is not the primary action, do not make it big and red. Make it a secondary/tertiary link. Use the red/bold styling only for the confirmation modal where it _is_ the primary action.
