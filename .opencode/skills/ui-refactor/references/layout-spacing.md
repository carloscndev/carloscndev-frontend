...
- **The System**: Start with a base (e.g., 16px). Create a scale where no two values are closer than ~25%.
  - _Example Scale_: 4, 8, 12, 16, 24, 32, 48, 64, 96, 128.
...
- **Separate groups**: Ensure space _between_ groups is larger than space _within_ groups.
  - _Ambiguity_: If a label's bottom margin equals the input's bottom margin, the relationship is unclear. Double the space between the input and the next label.
...
- **Grids are Overrated**: Do not force everything into percentage-based columns (e.g., 12-column grids).
  - _Sidebars_: Should usually be fixed width (optimized for their content), while the main content flexes.
- **Responsive Design**:
  - Use `max-width` rather than percentages. Only shrink elements when the screen gets smaller than the element's ideal size.
  - **Relative sizing fails**: Do not use `em` for layout widths. Elements large on desktop should shrink _faster_ than small elements on mobile. Sizing relationships are not linear across viewports.
