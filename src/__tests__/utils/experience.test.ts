...
describe("experience", () => {
  ...
  it("should update mobile carousel slides by index", async () => {
    document.body.innerHTML = "";
    const data = {
      es: {
        title: "Experiencia",
    ...
    expect(slides[0].querySelector("[data-job-role]")!.textContent).toBe(
      "Engineer EN",
    );
    expect(slides[0].querySelector("[data-job-company]")!.textContent).toBe(
      "@Acme EN",
    );
    expect(slides[0].querySelector("[data-job-period]")!.textContent).toBe(
      "2020–2021 EN",
    );
    expect(slides[0].querySelector("[data-job-description]")!.innerHTML).toBe(
      "<p>Paragraph 1 EN</p>",
    );
    const pills0 = slides[0].querySelectorAll("[data-job-tags] .tech-pill");
    expect(pills0[0].textContent).toBe("JS");
    expect(pills0[1].textContent).toBe("TS");

    expect(slides[1].querySelector("[data-job-role]")!.textContent).toBe(
      "Lead EN",
    );
    expect(slides[1].querySelector("[data-job-company]")!.textContent).toBe(
      "@Globex EN",
    );
    expect(slides[1].querySelector("[data-job-description]")!.innerHTML).toBe(
      "<p>Paragraph 2 EN</p>",
    );
    const pills1 = slides[1].querySelectorAll("[data-job-tags] .tech-pill");
    expect(pills1[0].textContent).toBe("Astro");
  });
});
