...
function updateAboutContent(lang: Lang): void {
  ...
  if (techContainer && data.technologies) {
    techContainer.innerHTML = data.technologies
      .map(
        (t) =>
          `<span class="about-section__tech-chip">${DIAMOND_ICON_HTML}<span>${t.name}</span></span>`,
      )
      .join("");
  }
