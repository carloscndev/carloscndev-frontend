...
export function updateLangUI(newLang: Lang): void {
  ...
  // Update aria-label on all language toggles
  const TOGGLE_IDS = ["language-toggle", "language-toggle-mobile"];
...
export function initLangListener(): void {
  ...
  document.addEventListener("astro:after-swap", () => {
    ...
    const lang = storedLang || domLang;

    if (storedLang && storedLang !== domLang) {
      document.documentElement.lang = storedLang;
    }
